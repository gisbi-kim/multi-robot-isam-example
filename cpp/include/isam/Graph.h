...
public:
  std::list<std::pair<Node*, std::string>> _nodes_key;
protected:
...
public:
  ...
  virtual void add_node(Node* node, std::string key) {
    _nodes.push_back(node);
    _nodes_key.push_back(std::make_pair(node, key));
    std::cout << "A node is added with the key: " << key << std::endl;
  }
